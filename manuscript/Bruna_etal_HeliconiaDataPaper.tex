...
\emph{Word count}: 4824 (text), 1734 (references)
...
\begin{tabular}{>{\raggedright\arraybackslash}p{5em}>{\raggedright\arraybackslash}p{16em}>{\raggedright\arraybackslash}p{22em}>{\raggedright\arraybackslash}p{3em}}
\toprule
...
\midrule
ranch & Ranch in which a plot is located & porto alegre
  
esteio
  
dimona & string\\
\midrule
bdffp\_no & The ID code assigned by the BDFFP to a reserve & 1104, 1202, 1301, 1501, 2107, 2108, 2206, 3209, 3402, NA: For an explanation of the numbering scheme see Gascon and Bierregaard 2001. `NA` indicates a plot is not inside the boundaries of a demarcated BDFFP reserve. & string\\
\midrule
yr\_isolated & For fragments, the year they were initially isolated & 1980, 1983, 1984 & integer\\
\bottomrule
\end{tabular}

\resizebox{\linewidth}{!}{
\begin{tabu} to \linewidth {>{\raggedright\arraybackslash}p{9em}>{\raggedright\arraybackslash}p{16em}>{\raggedright\arraybackslash}p{21em}>{\raggedright\arraybackslash}p{3em}}
\toprule
Variable & Definition & Codes or Range of Values (Units, Precision) & Storage\\
\midrule
plot & Plot in which plant is located & FF1-FF7, CF1-CF6 & string\\
\midrule
subplot & Subplot in which plant is located & A1-E10 in all plots except CF3, where F6-J10 & string\\
\midrule
...
\midrule
infl & if flowering, the no. of inflorescences & range = 1-7 (units = shoots, precision = 1) & integer\\
\midrule
recorded\_sdlg & Plant was new seedling that year & TRUE, FALSE & logical\\
\midrule
found\_without\_tag & Plant was established, post-seedling individual with no tag & TRUE, FALSE & logical\\
\midrule
treefall\_status & Plants under fallen trees trunks or crowns, branches, or leaf litter & branch: under fallen branches or tree limbs
